...
exception_recipient = ask("Exception recipient email: ")
sender_address = ask("sender address email: ")
...
Ubiquo::Config.add do |config|
  ...
  when 'development', 'test'
    config.notifier_email_from = '#{sender_address}'
  else
    config.notifier_email_from = '#{sender_address}'
  end
end
...
add_plugins(EXTERNAL_PLUGINS + UBIQUO_PLUGINS, :branch => "0.7-stable")
...
puts "1.- You must configure your databases using config/database.yml."
puts "2.- You must initialize your database. rake ubiquo:db:init"
puts "3.- You must create the admin user. rake ubiquo:create_user"
puts "4.- Enjoy it!"
